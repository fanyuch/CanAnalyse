...
BEGIN_MESSAGE_MAP(CFileView, CDockablePane)
	ON_MESSAGE(WM_USER_SHOWTREE, &ShowTreeView)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_CONTEXTMENU()
...
LRESULT CFileView::ShowTreeView(WPARAM p, LPARAM q)
{
	CList<CString, CString&> fileLst;
	CString pPath = *(CString *)p;
	HTREEITEM hRoot = m_wndFileView.InsertItem(pPath, 0, 0);
	m_wndFileView.SetItemState(hRoot, TVIS_BOLD, TVIS_BOLD);

	CFileFind finder;
	BOOL bNotEnd = finder.FindFile(pPath + _T("\\*.txt"));
	while (bNotEnd)
	{
		...
		if (!strTmp.IsEmpty())
		{
			m_wndFileView.InsertItem(strTmp, 1, 1, hRoot);
		}		
	}
	m_wndFileView.Expand(hRoot, TVE_EXPAND);
	return 0;
